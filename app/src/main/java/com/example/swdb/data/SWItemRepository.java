...
public class SWItemRepository implements SWItemAsyncTask.Callback {

    private final static String TAG = SWItemRepository.class.getSimpleName();

    private MutableLiveData<SWUtils.SWItemResult> mResults;

    private String mCurrentQuery;

    public SWItemRepository() {
    ...
    public void loadItemResults(String query) {
        if (shouldExecuteSearch(query)) {
            mCurrentQuery = query;
            new SWItemAsyncTask(query, this).execute();
        } else {
